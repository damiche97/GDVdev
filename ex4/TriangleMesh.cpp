...
void TriangleMesh::rotX(float factor) {
...
void TriangleMesh::rotY(float factor) {
  for (auto & vertex : vertices) {
    vertex.rotY(factor);
  }
...
void TriangleMesh::rotZ(float factor) {
  for (auto & vertex : vertices) {
    vertex.rotZ(factor);
  }
...
void TriangleMesh::translate(Vec3f const & translationVec) {
  for (auto & vertex : vertices) {
    vertex += translationVec;
  }
...
void TriangleMesh::calculateTexCoordsSphereMapping() {
  ...
  std::transform( // map each vertex to its texture coordinates
      ...
      [this](Vertex const & vertex) {
        const auto & modelCenter = boundingBoxMid;
