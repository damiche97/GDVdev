...
TriangleMesh::TriangleMesh(Vec3f ** rhs) {

}

TriangleMesh::TriangleMesh(TriangleMesh const & rhs):
  ...
  maybeTextureId(rhs.maybeTextureId),

  VBOsupported(rhs.VBOsupported),
  ...
  withColorArray(rhs.withColorArray),
  withBB(rhs.withBB),
  withNormals(rhs.withNormals),
  withTexture(rhs.withTexture),

  boundingBoxMin(rhs.boundingBoxMin),
  boundingBoxMax(rhs.boundingBoxMax),
  boundingBoxMid(rhs.boundingBoxMid),
  boundingBoxSize(rhs.boundingBoxSize)
{
  createAllVBOs();
}
...
std::vector<TriangleMesh::Vertex> const & TriangleMesh::getVertices() const {
  return vertices;
}
...
void TriangleMesh::loadFromMemory(
  ...
  triangles = std::move(inputTriangles);
...
void TriangleMesh::setColors(std::vector<Color> && colors) {
  // clear VBO data
  cleanupVBO();
  checkVBOSupport();
...
void TriangleMesh::calculateTexCoordsSphereMapping() {
  ...
  std::transform( // map each vertex to its texture coordinates
      std::begin(vertices),
      std::end(vertices),
      std::begin(texCoords), // write results directly in texCoords vector
      [this](Vertex const & vertex) {
        const auto modelCenter = getBoundingBoxMid();
        // compute representation of vector in model coordinates
        const auto [x, y, z] = vertex - modelCenter;

        // compute texture coordinates as explained in exercise sheet
        const float u = ((atan2(x, z)/M_PI) + 1)/2;
        const float v = asin(y / sqrt(x*x + y*y + z*z))/M_PI + 0.5;
...
void TriangleMesh::setWithColorArray(bool enable) {
  withColorArray = enable;
  cout << "setWithColorArray: " << withColorArray << endl;
}
...
void TriangleMesh::setTextureId(GLuint textureId) {
  maybeTextureId = std::make_optional(textureId);
}
...
void TriangleMesh::drawImmediate() {
  ...
  if (withTexture && texCoords.size() == vertices.size()) {
    ...
    if (maybeTextureId) {
      glBindTexture(GL_TEXTURE_2D, *maybeTextureId);
    }
  }
